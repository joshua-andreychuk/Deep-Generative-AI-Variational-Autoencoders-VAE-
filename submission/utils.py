...
def sample_gaussian(m, v):
    ...
    ################################################################################
    ### START CODE HERE ###
    # Sample standard normal noise ε with same shape as m
...
def log_normal(x, m, v):
    ...
    ### START CODE HERE ###
    ### START CODE HERE ###
    # single‐log version (stays entirely in PyTorch float32):
    #   log N(x; m, v) = –½[log(2π v) + (x–m)²/v]
    log_prob = -0.5 * (torch.log(2 * torch.pi * v) + (x - m).pow(2) / v)
...
def log_normal_mixture(z, m, v):
    ...
    ### START CODE HERE ###
    ### START CODE HERE ###
    # component log‐densities → (batch, k)
    comp_log = log_normal(z.unsqueeze(1), m, v)
    # subtract the uniform weight log(1/k) = –log k, computed in NumPy then cast
    k = m.size(1)
    comp_log = comp_log - torch.tensor(np.log(k),
                                       dtype=comp_log.dtype,
                                       device=comp_log.device)
    # log-sum-exp over mixture components → (batch,)
    return log_sum_exp(comp_log, dim=1)
    ### END CODE HERE ###
    ### END CODE HERE ###
    ################################################################################
    # End of code modification
