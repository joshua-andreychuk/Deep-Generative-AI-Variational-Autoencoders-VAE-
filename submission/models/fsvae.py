...
class FSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x, y):
        ...
        # 1) encode q(z | x, y)
        qm, qv = self.enc(x, y)  # -> (B, z_dim)

        # 2) sample one z per (x, y)
        z = ut.sample_gaussian(qm, qv)  # -> (B, z_dim)

        # 3) decode: p(x | y, z)
        mu = self.dec(z, y)  # -> (B, dim)

        # 4) reconstruction term: assume variance = 1/10
        #    log p(x|y,z) = log Normal(x; mu, diag(1/10))
        v = torch.ones_like(x) * 0.1
        log_px = ut.log_normal(x, mu, v)  # (B,)
        rec = -log_px.mean()

        # 5) KL divergence for z: D_KL(q(z|x,y) || p(z))
        pm = self.z_prior[0].expand(B, self.z_dim)
        pv = self.z_prior[1].expand(B, self.z_dim)
        kl_z = ut.kl_normal(qm, qv, pm, pv).mean()
